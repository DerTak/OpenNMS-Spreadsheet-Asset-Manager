...
public class Starter {
    ...
    public void doMain(String[] args) {
        ...
        logger.info("OpenNMS Asset Provisioning");
        ...
        if (m_generateOds) {
            ...
            if (allForeignSources) {
                RestAssetReader.generateAllOdsFiles(connParm, templateOdsFile);
            } else {
                if (m_foreignSource != null && !m_foreignSource.isEmpty()) {
                    RestAssetReader.generateOdsFile(m_foreignSource, connParm, templateOdsFile);
                } else {
            ...
            if (m_foreignSource != null && !m_foreignSource.isEmpty()) {
                if (m_odsFileSource != null && !m_odsFileSource.isEmpty()) {
                    RestAssetProvisioner.importAssetsFromOds(connParm, m_foreignSource, m_synchronize, m_odsFileSource);
                } else {
                    logger.error("To change assets on nodes from a ODS file, a ODS file is required");
                    parser.printUsage(System.err);
                }
            } else {
                logger.error("To change assets on nodes from a ODS file, a foreignsource is required.");
                parser.printUsage(System.err);
            }
        }
