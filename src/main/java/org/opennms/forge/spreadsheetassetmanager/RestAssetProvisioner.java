...
/**
 * <p>RestAssetProvisioner class.</p>
 * The main program to manage assets in provisioning via rest powered by spreadsheets.
 *
 ...
 * @since 1.0-SNAPSHOT
 */
public class RestAssetProvisioner {

    /**
     * Logging
     */
    private static Logger logger = LoggerFactory.getLogger(RestAssetProvisioner.class);

    /**
     * <p>importAssetsFromOds</p>
     * <p/>
     * Import nodes with spreadsheet provisioned assets into OpenNMS.
     *
     * @param restConnectionParameter helper that keeps base URL, username, password and so on for rest communication with OpenNMS
     * @param foreignSource           name of the foreign source to update note to asset mappings
     ...
     */
    public static void importAssetsFromOds(RestConnectionParameter restConnectionParameter, String foreignSource, Boolean synchronize, String filename) {
        ...
        Collection<NodeToAssetMapping> nodeToAssetMappings = readNodeToAssetMappingsFromOdsFile(odsFile);

        changeNodeToAssetMappingsInManagedRequisition(nodeToAssetMappings, requisitionManager);
     ...
     */
    private static Collection<NodeToAssetMapping> readNodeToAssetMappingsFromOdsFile(File odsFile) {

        Collection<NodeToAssetMapping> nodeToAssetMappings = new ArrayList<NodeToAssetMapping>();

        //read node to category mappings from spreadsheet
        SpreadsheetAssetReader spreadsheetReader;

        try {
            spreadsheetReader = new SpreadsheetAssetReader(odsFile);
            nodeToAssetMappings = spreadsheetReader.getNodeToAssetMappingsFromFile();

        } catch (IOException e) {
            logger.error("Error on reading spreadsheet with from '{}'.", odsFile.getAbsoluteFile(), e);
        }

        return nodeToAssetMappings;
    }

    private static void changeNodeToAssetMappingsInManagedRequisition(Collection<NodeToAssetMapping> nodeToAssetMappings, RequisitionManager requisitionManager) {

        for (NodeToAssetMapping nodeToAssetMapping : nodeToAssetMappings) {
            RequisitionNode requisitionNode = requisitionManager.getRequisitionNode(nodeToAssetMapping.getNodeLabel());
            if (requisitionNode != null) {

                //add all set assets
                for (RequisitionAsset addAsset : nodeToAssetMapping.getAddAssets()) {
                    requisitionNode.putAsset(addAsset);
                }

                //remove all not set assets
                for (RequisitionAsset removeAsset : nodeToAssetMapping.getRemoveAssets()) {
                    requisitionNode.deleteAsset(removeAsset);
                }

            } else {
                logger.info("RequisitionNode '{}' is unknown on the system", nodeToAssetMapping.getNodeLabel());
            }
        }
    }
}